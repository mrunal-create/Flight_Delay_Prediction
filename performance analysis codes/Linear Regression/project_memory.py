...
def analyze_memory_impact(memory_levels, file_path, parquet_path, target_column='DepDelay'):
    cleaning_times = []
    training_times = []

    for memory in memory_levels:
        print(f"Testing with {memory} executor memory...")

        # Initialize SparkSession with dynamic memory configuration
        spark = SparkSession.builder \
            .appName(f'project-{memory}-memory') \
        ...
        print(f"Cleaning time with {memory}G memory: {cleaning_time:.2f} seconds")
        ...
        print(f"Training time with {memory}G memory: {training_time:.2f} seconds")
        print(f"RMSE with {memory}G memory: {rmse:.2f}")
        
        spark.stop()

    # Plot results
    plt.figure(figsize=(10, 6))
    plt.plot(memory_levels, cleaning_times, marker='o', label='Cleaning Time')
    plt.plot(memory_levels, training_times, marker='x', label='Training Time')
    plt.title('Performance Analysis with Changing Memory')
    plt.xlabel('Executor Memory (GB)')
    plt.ylabel('Time (seconds)')
    plt.legend()
    plt.grid(True)
    plt.savefig("memory_impact_analysis.png", dpi=300, bbox_inches="tight")
    plt.show()

# Run memory impact analysis
memory_levels = [2, 4, 8, 16]
file_path = "concatenated_data.csv"
parquet_path = "concatenated_data.parquet"
analyze_memory_impact(memory_levels, file_path, parquet_path, target_column='DepDelay')
