...
def experiment_with_configurations(partition_size: int, memory: int, executor_cores: int,
    ...
    """
    # Creating a new Spark session with the desired configurations
    spark = create_spark_session(f"{memory}g", executor_cores, executor_instances, task_cpus, partition_size)

    # Logging the configurations being used
...
results = []
configurations = [
    (200, 4, 1, 1, 1),  # Partition Size: 200, Memory: 4GB, 1 core per executor, 1 executors, 1 CPU per task
    (400, 6, 2, 2, 2),  # Partition Size: 400, Memory: 6GB, 2 cores per executor, 2 executors, 2 CPU per task
    (800, 8, 2, 4, 3),  # Partition Size: 800, Memory: 8GB, 2 cores per executor, 4 executors, 3 CPUs per task
    (1600, 16, 4, 4, 4),  # Partition Size: 1600, Memory: 16GB, 4 cores per executor, 4 executors, 4 CPUs per task
]
...
# Convert the list of results into a pandas DataFrame for analysis/plotting
